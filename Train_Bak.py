...
from Code.utils.metricfunctions import iou,dice_coef
...
X_train = np.zeros((len(ids_train_x), im_height, im_width, 1), dtype=np.float32)
y_train = np.zeros((len(ids_train_y), im_height, im_width, 1), dtype=np.float32)

X_test = np.zeros((len(ids_test_x), im_height, im_width, 1), dtype=np.float32)
...
for x in (ids_train_x):
    y = glob.glob(x[:-4]+'*')[0]
    x_img = img_to_array(load_img(x, color_mode='grayscale', target_size=[im_width,im_height]))
    # Load masks
...
plt.ylabel("iou")
plt.legend();
plt.savefig('./Results/plots/'+str(args.Model)+'/train_iou.png')
